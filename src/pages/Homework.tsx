...
export const HomeworkPage: React.FC = () => {
    ...
    Array.from(files).forEach(file => {
      ...
      reader.onload = ev => {
        const att: HomeworkAttachment = {
          id: `ha${Date.now()}-${Math.random()}`, name: file.name,
          url: ev.target?.result as string, type: file.type, size: file.size,
        };
  ...
  const visible = homeworks.filter((h: any) => {
  ...
  const students = mockUsers.filter((u: any) => u.type === UserType.Student);
  ...
  const submitHW = async () => {
    ...
    try {
      await db.createHomework({
        studentId: currentUser.id, chapter: form.chapter,
        content: form.content, type: form.type,
      ...
      setFormError(err?.message || t('Something went wrong. Please try again.', 'حدث خطأ. حاول مرة أخرى.'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const deleteHW = async (id: string) => {
    if (!window.confirm(t('Delete this submission?', 'حذف هذا التسليم؟'))) return;
    setDeleting(p => ({ ...p, [id]: true }));
    try {
      await db.deleteHomework(id);
    } catch (err: any) {
      alert(err?.message || t('Failed to delete.', 'فشل الحذف.'));
    } finally {
  ...
  const addComment = async (hwId: string) => {
    ...
    try {
      ...
      setCommentErrors(p => ({ ...p, [hwId]: err?.message || t('Failed to save feedback.', 'فشل حفظ التعليق.') }));
    } finally {
  ...
  const canEditComment = (authorId: string) =>
    isAdmin || (currentUser.type === UserType.Teacher && authorId === currentUser.id);
  ...
  return (
    <Page>
      ...
      <PageHeader
        ...
        sub={isStudent
          ? t('Submit and track your homework', 'سلّم واجباتك وتابعها')
          : t('Review and comment on submissions', 'راجع التسليمات وعلّق عليها')}
        action={isStudent
          ? <Btn variant="primary" onClick={() => setShowForm(!showForm)}>+ {t('Submit Homework', 'تسليم واجب')}</Btn>
          : undefined}
      />
      ...
      {showForm && isStudent && (
        <Card style={{ marginBottom: 24, border: '1.5px solid #c7d2fe', animation: 'fadeUp 0.3s ease forwards' }}>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {(['homework', 'exam'] as const).map(ty => (
                <button key={ty} onClick={() => setForm(f => ({ ...f, type: ty, chapter: '' }))}
                  style={{ flex: 1, padding: '9px 14px', borderRadius: 9, cursor: 'pointer', fontSize: 13.5, fontWeight: 600, fontFamily: 'inherit', border: `1.5px solid ${form.type === ty ? '#c7d2fe' : '#e5e7eb'}`, background: form.type === ty ? '#eef2ff' : '#fff', color: form.type === ty ? '#4338ca' : '#6b7280' }}>
          ...
          {form.type === 'homework' ? (
            <div style={{ marginBottom: 14 }}>
              ...
              <select value={form.chapter} onChange={e => setForm(f => ({ ...f, chapter: e.target.value }))}
                style={{ width: '100%', padding: '10px 12px', border: '1px solid #d1d5db', borderRadius: 9, fontSize: 14, fontFamily: 'inherit', color: '#111827', background: '#fff', outline: 'none' }}>
                <option value="">{t('Select chapter…', 'اختر فصلاً…')}</option>
                {['Correlation','Coding','Venn Diagram','Tree Diagram','Boxplot & Stem and Leaf','Continuous Data','Histogram','Discrete Basic','Discrete Tricky','Discrete Level El Wash','Normal Distribution Basics','Normal Distribution Simultaneous & Given That'].map(ch => <option key={ch} value={ch}>{ch}</option>)}
              </select>
            </div>
          ) : (
            <div style={{ marginBottom: 14 }}>
              ...
              <input value={form.chapter} onChange={e => setForm(f => ({ ...f, chapter: e.target.value }))}
                placeholder={t('e.g. Mock Exam 1', 'مثال: امتحان تجريبي 1')}
          ...
          <div style={{ marginBottom: 14 }}>
            <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#374151', marginBottom: 8 }}>{t('Attach Files', 'إرفاق ملفات')}</label>
          ...
          {formError && <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 9, padding: '10px 14px', color: '#b91c1c', fontSize: 13, marginBottom: 14 }}>{formError}</div>}
      ...
      {isTeacher && (
        <>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
            {[{ id: 'all', name: t('All Students', 'جميع الطلاب') }, ...students].map((s: any) => (
              <button key={s.id} onClick={() => setFilterStudent(s.id)} style={{ padding: '6px 14px', borderRadius: 99, border: `1.5px solid ${filterStudent === s.id ? '#c7d2fe' : '#e5e7eb'}`, background: filterStudent === s.id ? '#eef2ff' : '#fff', color: filterStudent === s.id ? '#4338ca' : '#6b7280', cursor: 'pointer', fontSize: 13, fontWeight: 600, fontFamily: 'inherit' }}>
                {s.name}
              </button>
            ))}
          </div>
      ...
      {/* Homework list */}
      {visible.length === 0
        ? <Card><Empty text={t('No homework yet', 'لا توجد واجبات بعد')} /></Card>
        : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {visible.map((h: any, i: number) => {
              const student = mockUsers.find((u: any) => u.id === h.studentId);
              return (
                          ...
                          {(isAdmin || (isStudent && h.studentId === currentUser.id)) && (
                            <button
                              ...
                              style={{
                                ...
                                fontWeight: 700,
                                fontFamily: 'inherit',
                                opacity: deleting[h.id] ? 0.6 : 1,
                                transition: 'all 0.15s',
                              }}>
                              {deleting[h.id] ? t('Deleting…', 'جارٍ الحذف…') : '🗑 ' + t('Delete', 'حذف')}
                            </button>
                          )}
                        </div>
                      </div>
                      ...
                      {h.comment && !editingComment[h.id] ? (
                        <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 10, padding: '12px 14px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                            <span style={{ fontSize: 13, fontWeight: 700, color: '#065f46' }}>{t("Mena's feedback:", 'تعليق منى:')}</span>
                            {canEditComment(h.comment.teacherId) && (
                              <button onClick={() => { setCommentForms(p => ({ ...p, [h.id]: h.comment!.content })); setEditingComment(p => ({ ...p, [h.id]: true })); }}
                        ...
                        <div>
                          <textarea value={commentForms[h.id] || ''} onChange={e => setCommentForms(p => ({ ...p, [h.id]: e.target.value }))}
                            placeholder={t('Write feedback…', 'اكتب تعليقاً…')} rows={2}
                          ...
                          {commentErrors[h.id] && <div style={{ color: '#b91c1c', fontSize: 12, marginTop: 8 }}>{commentErrors[h.id]}</div>}
                        </div>
                      ) : (
