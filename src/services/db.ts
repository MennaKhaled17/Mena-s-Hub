...
//
// Looks up ALL matching rows (not .maybeSingle()) because some homework
// items already accumulated more than one comment row from before this
// upsert behavior existed — .maybeSingle() throws "multiple rows returned"
// in that case. The most recent row is updated, and any older duplicates
// for the same homework are cleaned up so this can't recur.
//
...
export async function addHomeworkComment(
  ...
  if (existingRows && existingRows.length > 0) {
    ...
    if (duplicates.length > 0) {
      await supabase.from('homework_comments').delete().in('id', duplicates.map(d => d.id));
    }
  } else {
    const { data, error } = await supabase
      .from('homework_comments')
